...
from utils import EarlyStopping, check_path, set_seed, get_user_seqs, generate_rating_matrix, get_user_seqs_from_expo, \
    read_tsv_data, augment_data
...
def main():
    ...
    parser.add_argument('--dro_reg', default=1, type=float, help="-1 IPS, -2 IPS-C -3 RelMF")
    ...
    if 'MB' not in args.data_name:
        ...
        _, train_data, _, max_item, _ = read_tsv_data(
            args.data_file + "/raw_data/retail_argumented_False_seqaug_False.train.inter")
        if args.do_augment:
            train_data = augment_data(train_data)
        item_size = max(item_size, max_item)
        _, valid_data, _, max_item, _ = read_tsv_data(
            args.data_file + "/raw_data/retail_argumented_False_seqaug_False.val.inter")
        item_size = max(item_size, max_item)
        _, test_data, _, max_item, _ = read_tsv_data(
            args.data_file + "/raw_data/retail_argumented_False_seqaug_False.test.inter")
        item_size = max(item_size, max_item)
        args.item_size = item_size + 2
    ...
    if args.augmentation_ratio != 0:
        if not os.path.exists(os.path.join(args.data_file, f'augment_exposure-ratio{args.augmentation_ratio}.txt')):
            raise FileNotFoundError(f"Error: Augmentation ratio {args.augmentation_ratio} does not exist.")
        ori_user_seq, user_seq, max_item, max_len, item_counter = get_user_seqs_from_expo(
            os.path.join(args.data_file, f'augment_exposure-ratio{args.augmentation_ratio}.txt'))
        item_size = max(item_size, max_item)
        print(user_seq)
        train_data += user_seq
    ...
    trainer = Trainer(model, train_dataloader, eval_dataloader,
                      test_dataloader, args, exposure_model, evaluation_model)
